...
// The filename is used for #line directives.
...
type generator struct {
	filename  string
	indent    int
	funcCount int               // counter for generating unique function names
...
func (g *generator) generate(prog *syntax.Program) string {
	...
	// (they need to be emitted before main so they can reference each other)
...
func (g *generator) emitVarDecl(s *syntax.VarDeclStmt) {
	...
	if fnExpr, isFn := s.Value.(*syntax.FuncExpr); isFn {
		...
		// No variable needed in main — calls go directly to the C function
		return
	}
...
func (g *generator) emitAssign(s *syntax.AssignStmt) {
	...
	case *syntax.IdentExpr:
		...
		if s.Op == syntax.Equal {
			...
			g.emitLine("    %s = monk_deep_copy(%s);\n", name, value)
		} else {
			op := compoundToArith(s.Op)
			g.emitLine("    { MonkValue _tmp = %s(%s, %s);\n", op, name, value)
			g.emitLine("      monk_free(%s);\n", name)
			g.emitLine("      %s = _tmp; }\n", name)
		}
...
func (g *generator) emitIf(s *syntax.IfStmt) {
	...
	if s.Else != nil {
		...
		case *syntax.IfStmt:
			g.emitLine("    } else ")
			// Emit inline (no extra indent)
			cond := g.emitExpr(e.Condition)
			g.body.WriteString(fmt.Sprintf("if (monk_is_truthy(%s)) {\n", cond))
			for _, stmt := range e.Then.Stmts {
				g.emitStmt(stmt)
			}
			if e.Else != nil {
				g.emitLine("    } else {\n")
				if block, ok := e.Else.(*syntax.BlockStmt); ok {
...
func (g *generator) emitFor(s *syntax.ForStmt) {
	...
	g.emitLine("                while ((_iter.str_val[_i + _clen] & 0xC0) == 0x80) _clen++;\n")
...
func (g *generator) emitGuard(s *syntax.GuardStmt) {
	...
	g.emitLine("            (void)%s;\n", errName) // suppress unused warning
	g.emitLine("        }\n")
	g.emitLine("    }\n")
}
...
func (g *generator) emitExpr(expr syntax.Expr) string {
	...
	case *syntax.IndexExpr:
		...
		// Runtime handles both array and string indexing
		return fmt.Sprintf("((%s).kind == MONK_STRING ? monk_string_index(%s, %s) : monk_array_get(%s, %s))",
			obj, obj, idx, obj, idx)
...
func (g *generator) emitBinary(e *syntax.BinaryExpr) string {
	...
	// Arithmetic
	case syntax.Plus:
		// Could be string concat or addition — runtime decides
		return fmt.Sprintf("((%s).kind == MONK_STRING ? monk_string_concat(%s, %s) : monk_add(%s, %s))",
...
func (g *generator) emitCall(e *syntax.CallExpr) string {
	...
	if ident, ok := e.Callee.(*syntax.IdentExpr); ok {
		...
		// Unknown — might be a forward reference or passed-in function
		// Fall through to the hoisted name convention
		return fmt.Sprintf("%s(%s)", mangleName(ident.Name), argStr)
	}

	// Indirect call (method on record, etc.) — not yet supported
...
func (g *generator) emitFuncExpr(e *syntax.FuncExpr) string {
	// Anonymous function expression (not bound to a named variable).
	// Hoist it and return a reference. For now, anonymous closures are limited.
	g.funcCount++
...
func mangleName(name string) string {
	// Prefix all user names with mk_ to avoid colliding with C keywords
	// or runtime function names
	return "mk_" + name
}
